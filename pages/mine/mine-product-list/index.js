...
Page({
  ...
  goRecordwrite(e) {
    ...
    request('/app/person/getBatchCard', 'post', {
      ...
      console.log(res);
      if (res.data.organics == "" && res.data.tests == "" && res.data.elses == "") {
        wx.navigateTo({
          url: `/pages/mine/mine-product-add/index?batchId=${id}&type=${type}`
        })
      } else {
        wx.navigateTo({
          url: `/pages/mine/mine-product-result/index?batchId=${id}&type=${type}`
        })
      }
    })
  },

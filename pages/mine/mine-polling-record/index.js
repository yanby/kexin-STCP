...
Page({
  ...
  init(){
    request('/app/routing/getRouting', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.routing.routing_time,
        content: res.data.routing.text,
        imgs1: res.data.routing.imgs
      })
  ...
  delImg1(e) {
    ...
    if (id) {
      request('/app/routing/delImg', 'post', {
        id: id
      }).then(res => {
        console.log(res)
  ...
  resultFun(){
    var that = this;
    request('/app/routing/saveRouting', 'post', {
      "routing.routing_time": that.data.date, //农事时间
      "routing.type": this.data.coteType,//巡检批次类型
      "routing.farm": this.data.obj.farm || "",
      "routing.batch": this.data.obj.id || "",
      "routing.cote": this.data.obj.cote_crop || "",//巡检地块
      "routing.text": that.data.content,//文字信息
      "routing.status": that.data.status,//0 暂存 1 保存
      "imgs": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "routing.routing_user": wx.getStorageSync('token'),//巡检用户
      "routing.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

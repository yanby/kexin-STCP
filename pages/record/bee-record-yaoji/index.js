...
Page({
  ...
  init(){
    request('/app/batchBee/getDrugInput', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.drugInput.info_time,
        content: res.data.drugInput.text,
        imgs1: res.data.drugInput.drugFaces,
        imgs2: res.data.drugInput.drugCons,
        imgs3: res.data.drugInput.spots,
        dosage: res.data.drugInput.measure,
        object: res.data.drugInput.prevention_object,
        name: res.data.drugInput.drug_name
      })
  ...
  resultFun(){
    var that = this;
    request('/app/batchBee/saveDrugInput', 'post', {
      "batchBeeDrugInput.info_time": that.data.date, //农事时间
      "batchBeeDrugInput.text": that.data.content,//文字信息
      "batchBeeDrugInput.status": that.data.status,//0 暂存 1 保存
      "batchBeeDrugInput.batch_bee": that.data.batchId,//批次id
      ...
      "batchBeeDrugInput.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchBeeDrugInput.prevention_object": that.data.object,//防治对象
      "batchBeeDrugInput.drug_name": that.data.name,//药剂名称
      "batchBeeDrugInput.measure": that.data.dosage,//药剂量
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

...
Page({
  ...
  init(){
    request('/app/batchEgg/getToDay', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.batchEggToDay.info_time,
        content: res.data.batchEggToDay.text,
        imgs1: res.data.batchEggToDay.imgs,
        num: res.data.batchEggToDay.die_num,
        name: res.data.batchEggToDay.work_time
      })
  ...
  resultFun(){
    var that = this;
    request('/app/batchEgg/saveToDay', 'post', {
      "batchEggToDay.info_time": that.data.date, //农事时间
      "batchEggToDay.text": that.data.content,//文字信息
      "batchEggToDay.status": that.data.status,//0 暂存 1 保存
      "batchEggToDay.batch_egg": that.data.batchId,//批次id
      "imgs": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "batchEggToDay.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchEggToDay.die_num": that.data.num,//数量
      "batchEggToDay.work_time": that.data.name,//工时记录
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

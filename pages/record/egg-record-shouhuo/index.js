...
Page({
  ...
  init(){
    var that = this;
    request('/app/batchEgg/getRecover', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.batchEggRecovery.info_time,
        content: res.data.batchEggRecovery.text,
        imgs1: res.data.batchEggRecovery.imgs,
        dosage: res.data.batchEggRecovery.egg_num,
        num: res.data.batchEggRecovery.slay_num
      })
    })
  },
  ...
  resultFun(){
    var that = this;
    request('/app/batchEgg/saveRecover', 'post', {
      "batchEggRecovery.info_time": that.data.date, //农事时间
      "batchEggRecovery.text": that.data.content,//文字信息
      "batchEggRecovery.handle": this.data.name,//饲料名
      "batchEggRecovery.status": that.data.status,//0 暂存 1 保存
      "batchEggRecovery.batch_egg": that.data.batchId,//批次id
      "imgs": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "batchEggRecovery.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchEggRecovery.egg_num": that.data.dosage,//鸡蛋量
      "batchEggRecovery.slay_num": that.data.num,//宰杀量
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

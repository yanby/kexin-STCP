...
Page({
  ...
  init(){
    request('/app/batchBee/getToDay', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
  ...
  resultFun(){
    var that = this;
    request('/app/batchBee/saveToDay', 'post', {
      "batchBeeToDay.info_time": that.data.date, //农事时间
      "batchBeeToDay.text": that.data.content,//文字信息
      "batchBeeToDay.status": that.data.status,//0 暂存 1 保存

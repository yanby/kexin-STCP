...
Page({
  ...
  init(){
    request('/app/batchEgg/getForage', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.batchEggForage.info_time,
        content: res.data.batchEggForage.text,
        imgs1: res.data.batchEggForage.forageFaces,
        imgs2: res.data.batchEggForage.spots,
        dosage: res.data.batchEggForage.feed_num,
        name: res.data.batchEggForage.forage_name
      })
  ...
  bindPickerChange: function (e) {
    ...
    this.setData({
      ...
      siliaoId: this.data.forages[index].id
    })
    console.log(this.data.siliaoId)
  },
  //时间选择
  ...
  resultFun(){
    var that = this;
    request('/app/batchEgg/saveForage', 'post', {
      "batchEggForage.info_time": that.data.date, //农事时间
      "batchEggForage.text": that.data.content,//文字信息
      "batchEggForage.forage_name": this.data.name,//饲料名
      "batchEggForage.egg_forage": this.data.siliaoId,
      "batchEggForage.status": that.data.status,//0 暂存 1 保存
      "batchEggForage.batch_egg": that.data.batchId,//批次id
      "forageFaces": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "spots": JSON.stringify(that.data.imgs2),//	施药图片数组
      "batchEggForage.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchEggForage.feed_num": that.data.dosage,//药剂量
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

...
Page({
  ...
  init(){
    request('/app/batchCrop/getMuck', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.muck.info_time,
        content: res.data.muck.text,
        imgs1: res.data.muck.muckFaces,
        imgs2: res.data.muck.muckCons,
        imgs3: res.data.muck.spots,
        dosage: res.data.muck.muck_measure,
        name: res.data.muck.muck_name
      })
      if (res.data.muck.unit == "毫升") {
        this.setData({
          index: 1,
  ...
  resultFun(){
    var that = this;
    request('/app/batchCrop/saveMuck', 'post', {
      "batchCropMuck.info_time": that.data.date, //农事时间
      "batchCropMuck.text": that.data.content,//文字信息
      "batchCropMuck.status": that.data.status,//0 暂存 1 保存
      "batchCropMuck.batch_crop": that.data.batchId,//批次id
      "muckFaces": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "muckCons": JSON.stringify(that.data.imgs2),//药记反面图片数组
      "spots": JSON.stringify(that.data.imgs3),//	施药图片数组
      "batchCropMuck.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchCropMuck.muck_name": that.data.name,//药剂名称
      "batchCropMuck.muck_measure": that.data.dosage,//药剂量
      "batchCropMuck.unit": that.data.unit
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

...
Page({
  ...
  init(){
    request('/app/batchEgg/getDrugInput', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.batchEggDrugInput.info_time,
        content: res.data.batchEggDrugInput.text,
        imgs1: res.data.batchEggDrugInput.drugFaces,
        imgs2: res.data.batchEggDrugInput.drugCons,
        imgs3: res.data.batchEggDrugInput.spots,
        dosage: res.data.batchEggDrugInput.measure,
        object: res.data.batchEggDrugInput.prevention_object,
        name: res.data.batchEggDrugInput.drug_name
      })
  ...
  resultFun(){
    var that = this;
    request('/app/batchEgg/saveDrugInput', 'post', {
      "batchEggDrugInput.info_time": that.data.date, //农事时间
      "batchEggDrugInput.text": that.data.content,//文字信息
      "batchEggDrugInput.status": that.data.status,//0 暂存 1 保存
      "batchEggDrugInput.batch_egg": that.data.batchId,//批次id
      "drugFaces": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "drugCons": JSON.stringify(that.data.imgs2),//药记反面图片数组
      "spots": JSON.stringify(that.data.imgs3),//	施药图片数组
      "batchEggDrugInput.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchEggDrugInput.prevention_object": that.data.object,//防治对象
      "batchEggDrugInput.drug_name": that.data.name,//药剂名称
      "batchEggDrugInput.measure": that.data.dosage,//药剂量
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

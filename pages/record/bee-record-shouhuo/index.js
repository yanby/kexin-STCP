...
Page({
  ...
  init(){
    request('/app/batchBee/getRecovery', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      ...
      this.setData({
        ...
        imgs1: res.data.recovery.operateSpots,
        imgs2: res.data.recovery.spots,
        imgs3: res.data.recovery.labels,
        fengmi: res.data.recovery.mel,
        yujiang: res.data.recovery.king,
        huafeng: res.data.recovery.dust,
        fengla: res.data.recovery.wax,
        fengjiao: res.data.recovery.glue
      })
  ...
  resultFun(){
    var that = this;
    request('/app/batchBee/saveRecovery', 'post', {
      "batchBeeRecovery.info_time": that.data.date, //农事时间
      "batchBeeRecovery.text": that.data.content,//文字信息
      "batchBeeRecovery.status": that.data.status,//0 暂存 1 保存
      "batchBeeRecovery.batch_bee": that.data.batchId,//批次id
      "operateSpots": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "spots": JSON.stringify(that.data.imgs2),//药记反面图片数组
      "labels": JSON.stringify(that.data.imgs3),//	施药图片数组
      "batchBeeRecovery.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchBeeRecovery.mel": that.data.fengmi,
      "batchBeeRecovery.king": that.data.yujiang,
      "batchBeeRecovery.dust": that.data.huafeng,
      "batchBeeRecovery.wax": that.data.fengla,
      "batchBeeRecovery.glue": that.data.fengjiao
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

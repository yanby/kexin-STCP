...
Page({
  ...
  init() {
    ...
    request('/app/batchEgg/getDrugInput', 'post', {
      ...
      this.setData({
        toDay: res.data.batchEggDrugInput,
        batchInfo: res.data.batchInfo
      })

      if (res.data.batchEggDrugInput.status == 0) {
        this.setData({
          status: true
        })
      }
  ...
  addFun(e){
    ...
    wx.redirectTo({
      url: '/pages/record/egg-record-yaoji/index?listId=' + id
    })
  },
  //图片放大功能
  imgBig(e) {
  ...
  configFun() {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          request('/app/batchEgg/closureBatchInfo', 'post', {
            id: that.data.listId,
            type: "drug_input",
          }).then(res => {
            console.log(res);

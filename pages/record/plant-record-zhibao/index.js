...
Page({
  ...
  init(){
    request('/app/batchCrop/getProtection', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.protection.info_time,
        content: res.data.protection.text,
        imgs1: res.data.protection.drugFaces,
        imgs2: res.data.protection.drugCons,
        imgs3: res.data.protection.spots,
        dosage: res.data.protection.measure,
  ...
  resultFun(){
    var that = this;
    request('/app/batchCrop/saveProtection', 'post', {
      "batchCropProtection.info_time": that.data.date, //农事时间
      "batchCropProtection.text": that.data.content,//文字信息
      "batchCropProtection.status": that.data.status,//0 暂存 1 保存
      "batchCropProtection.batch_crop": that.data.batchId,//批次id
      ...
      "batchCropProtection.id": that.data.listId,//巡查 id，无id为新增，有id为编辑

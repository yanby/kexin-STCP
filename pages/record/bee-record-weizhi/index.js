...
Page({
  ...
  init(){
    request('/app/batchBee/getLocation', 'post', {
      id: this.data.listId
    }).then(res => {
      console.log(res)
      
      this.setData({
        date: res.data.location.info_time,
        content: res.data.location.text,
        imgs1: res.data.location.imgs,
        address: res.data.location.new_location
      })
  ...
  resultFun(){
    var that = this;
    request('/app/batchBee/saveLocation', 'post', {
      "batchBeeLocation.info_time": that.data.date, //农事时间
      "batchBeeLocation.text": that.data.content,//文字信息
      "batchBeeLocation.status": that.data.status,//0 暂存 1 保存
      "batchBeeLocation.batch_bee": that.data.batchId,//批次id
      "imgs": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "batchBeeLocation.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchBeeLocation.new_location": that.data.address,//药剂名称
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

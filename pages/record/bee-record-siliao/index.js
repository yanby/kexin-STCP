...
Page({
  ...
  init(){
    request('/app/batchBee/getForage', 'post', {
      ...
      this.setData({
        date: res.data.forage.info_time,
        content: res.data.forage.text,
        imgs1: res.data.forage.forages,
        imgs2: res.data.forage.spots,
        dosage: res.data.forage.forage_measure,
        name: res.data.forage.forage_name
      })
  ...
  resultFun(){
    var that = this;
    request('/app/batchBee/saveForage', 'post', {
      "batchBeeForage.info_time": that.data.date, //农事时间
      "batchBeeForage.text": that.data.content,//文字信息
      "batchBeeForage.forage_name": this.data.name,//饲料名
      "batchBeeForage.status": that.data.status,//0 暂存 1 保存
      "batchBeeForage.batch_bee": that.data.batchId,//批次id
      "forages": JSON.stringify(that.data.imgs1),//药记正面图片数组
      "spots": JSON.stringify(that.data.imgs2),//	施药图片数组
      "batchBeeForage.id": that.data.listId,//巡查 id，无id为新增，有id为编辑
      "batchBeeForage.forage_measure": that.data.dosage,//药剂量
      "batchBeeForage.forage_name": that.data.name
    }).then(res => {
      console.log(res);
      wx.navigateBack()
    })

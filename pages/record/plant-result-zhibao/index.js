...
Page({
  ...
  init() {
    ...
    request('/app/batchCrop/getProtection', 'post', {
      ...
      this.setData({
        toDay: res.data.protection,
        batchInfo: res.data.batchInfo
      })

      if (res.data.protection.status == 0) {
        this.setData({
          status: true
        })
      }
  ...
  addFun(e){
    ...
    wx.redirectTo({
      url: '/pages/record/plant-record-zhibao/index?listId=' + id
    })
  },
  //图片放大功能
  imgBig(e) {
  ...
  configFun() {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          request('/app/batchCrop/closureBatchInfo', 'post', {
            id: that.data.listId,
            type: "protection",
          }).then(res => {
            console.log(res);

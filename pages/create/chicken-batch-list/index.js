...
Page({
  ...
  init() {
    request('/app/batch/list', 'post', {
      ...
      console.log(res);
      this.setData({
        cropResult: res.data.eggResult.list,
  ...
  goChangeBatch(e) {
    ...
    wx.navigateTo({
      url: '/pages/create/chicken-change-batch/index?batch=' + o
    })
  },
  //跳转创建批次
  goCreatBatch() {
    wx.navigateTo({
      url: '/pages/create/chicken-creat-batch/index?plotId=' + this.data.plotId
    })
  },
  //点击删除
  delFun(e) {
  ...
  onReachBottom: function () {
    ...
    request('/app/batch/list', 'post', {
      ...
      that.setData({
        cropResult: that.data.cropResult.concat(res.data.eggResult.list),
        nextPage: res.data.eggResult.nextPage
      })
    })
  },
